...
def FermiDirac(E, T):
    """Fermi Dirac function with E in eV and T in Kelvin"""
...
class NaturalOrbitals:
    ...
    def get_NAO(self, threshold=1.90):
        ...
        print("Number of electron pairs: {}".format(self.ne))

        self.HOMO = eig_v[int(self.ne - 1)]
        ...
        for n, atom in enumerate(self.mol):
            ...
            if atom.symbol in ['N', 'C', 'O']:
                if self.core_and_valence is True:
...
#           problem = []
    ...
    def get_currents(self, Vs, T):
        ...
        if self.transmission_uptodate is False:
    ...
    def get_conductances(self):
        if self.currents_uptodate is False:
